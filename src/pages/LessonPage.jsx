  ...
  const runQuizGeneration = async () => {
    ...
    try {
      if (rawBankQuestions && rawBankQuestions.length > 0) {
        ...
        const selectedPool = shuffledQuestions.slice(0, Math.min(10, shuffledQuestions.length));
        ...
        alert("Sila muatkan nota atau bank soalan terlebih dahulu.");
      }
    } catch (err) {
            ...
            <div className="relative z-10">
              ...
              <div className="max-w-md">
                <motion.div whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }}>
                  <Button onClick={runQuizGeneration} disabled={status.quiz} size="lg" className="bg-orange-500 hover:bg-orange-600 text-white h-16 text-sm font-bold rounded-2xl w-full border-b-4 border-orange-700 active:border-b-0 active:translate-y-1 transition-all">
                    {status.quiz ? <Loader2 className="w-5 h-5 animate-spin mr-2" /> : <Play className="w-5 h-5 mr-2 fill-current" />} Mula Kuiz Sekarang!
                  </Button>
                </motion.div>
              </div>
            </div>
