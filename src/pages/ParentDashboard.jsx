...
function SmartParentingTips() {
  ...
  return (
    <Card className="border-0 shadow-sm rounded-2xl bg-gradient-to-r from-indigo-900 to-purple-900 text-white overflow-hidden relative">
...
function WeatherCard() {
  const [weather, setWeather] = useState({ temp: 30, code: 61 });
  const [forecast, setForecast] = useState([]);
  const [location, setLocation] = useState("Kota Kinabalu");
  ...
  return (
    <Card className="p-5 rounded-2xl shadow-sm border-sky-100 bg-gradient-to-br from-blue-50 to-sky-100 relative overflow-hidden">
      <div className="flex justify-between items-start gap-4 relative z-10">
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-1 mb-1 text-sky-600">
            ...
            <p className="text-[10px] font-bold uppercase tracking-wider truncate">{location}</p>
          </div>
          <h3 className="text-3xl font-black text-slate-800 leading-none">{weather.temp}°<span className="text-xl">c</span></h3>
      ...
      <div className="mt-4 pt-4 border-t border-sky-200/60">
        <p className="text-[9px] font-bold text-sky-600 uppercase tracking-wider mb-2">Ramalan Seterusnya</p>
        <div className="grid grid-cols-5 gap-1 text-center">
          {["5 PM", "6 PM", "7 PM", "8 PM", "9 PM"].map((time, i) => (
            <div key={i} className="flex flex-col items-center min-w-0">
              <span className="text-[9px] text-slate-500 font-semibold truncate w-full">{time}</span>
              <CloudRain className="w-3.5 h-3.5 text-blue-400 my-1 shrink-0" />
              <span className="text-[10px] font-bold text-slate-700">{30 - i}°</span>
            </div>
          ))}
        </div>
      </div>
...
// ---------------- 4. KAD INDIVIDU ANAK (PERBAIKAN UTAMA TERTINDIH & OVERFLOW) ----------------
function ChildCard({ child, onRefresh }) {
  ...
  return (
    <>
      <Card className="p-4 bg-white border border-slate-100 rounded-2xl shadow-sm flex flex-col justify-between h-full min-h-[290px]">
        <div className="space-y-3.5">
          ...
          {/* SAKAL XP - Diperbaiki menggunakan susunan Row Justify-Between untuk elak bertindih */}
          ...
          {/* GRID TIGA KOTAK STATISTIK - Dioptimumkan ketinggian dan saiz teks */}
          <div className="grid grid-cols-3 gap-1.5">
            ...
            <div className="bg-indigo-50/40 border border-indigo-100/30 p-2 rounded-xl flex flex-col items-center justify-center text-center min-h-[62px] min-w-0">
              ...
              <p className="text-[10px] font-extrabold text-slate-700 mt-1 leading-tight truncate w-full px-0.5" title={currentTopic}>
                {currentTopic}
              </p>
            </div>
          </div>
        ...
        <div className="grid grid-cols-2 gap-2 border-t border-slate-100 pt-3 mt-3">
          <Button size="sm" variant="outline" className="h-7 text-[9px] font-black border-amber-200 text-amber-600 hover:bg-amber-50 rounded-lg shadow-2xs">
            🪙 Bonus Koin
          </Button>
          <Button size="sm" variant="outline" className="h-7 text-[9px] font-black border-indigo-200 text-indigo-600 hover:bg-indigo-50 rounded-lg shadow-2xs" onClick={() => setShowMissionModal(true)}>
            🎯 Misi Khas
          </Button>
        </div>
      </Card>
...
export default function ParentDashboard() {
  ...
  useEffect(() => {
    // Simulasi data persis tangkapan skrin user
    const mockData = [
      { id: "1", display_name: "IVANOVIC", progress: { level: 1, xp_score: 195, next_level_xp: 500, streak_days: 2, current_topic: "M..." }, wallet: { balance: 390 } },
      { id: "2", display_name: "CORRY A...", progress: { level: 2, xp_score: 260, next_level_xp: 1000, streak_days: 2, current_topic: "M..." }, wallet: { balance: 520 } },
      { id: "3", display_name: "MORRY", progress: { level: 2, xp_score: 295, next_level_xp: 1000, streak_days: 0, current_topic: "M..." }, wallet: { balance: 0 } }
    ];
  ...
  return (
    <div className="p-4 sm:p-5 lg:p-6 space-y-5 max-w-7xl mx-auto bg-slate-50/30 min-h-screen">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        ...
        <ShortcutCard icon={BarChart2} title="Analitik" desc="Semak perkembangan" gradient="from-blue-500 to-cyan-500" onClick={() => toast({ title: "Modul Analitik" })} />
      ...
      {/* STRUKTUR GRID KOMPUTER & TABLET */}
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-5 items-start">
        
        {/* KOLUM KIRI: SENARAI PROFIL ANAK (8/12 Ruang Desktop) */}
        <div className="lg:col-span-8 space-y-3.5">
          ...
          {/* Reka bentuk kad responsif - Menggunakan 2 kolum pada tablet/skrin besar, 3 pada skrin monitor besar */}
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-2 xl:grid-cols-3 gap-4">
            {children.map((child) => (
              <ChildCard key={child.id} child={child} onRefresh={()=>{}} />
            ))}
          </div>
        </div>

        {/* KOLUM KANAN: UTALITI DAN TIPS (4/12 Ruang Desktop) */}
