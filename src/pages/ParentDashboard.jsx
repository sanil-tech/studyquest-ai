...
function DetailedChildCard({ child, onOpenReport }) {
  ...
  // 🔗 Sinkronisasi data XP, Streak & Level dari entiti Progress murid
  ...
  // Membaca topik & masa dari StudySession berdasarkan fail CSV anda
...
export default function ParentDashboard() {
  ...
  const loadDashboardData = async () => {
    try {
      ...
      const kids = await Promise.all(childIds.map(async (id) => {
        // 🔥 MULTI-FILTER: Membaca StudySession DAN Progress untuk menyatukan data sebenar
  ...
  if (loading) {
    return <div className="p-6 text-center text-xs text-slate-400 font-medium">Memuatkan dashboard...</div>;
  }
    ...
    <div className="p-4 sm:p-6 space-y-4 max-w-7xl mx-auto bg-slate-50/30 min-h-screen">
      <div>
        <h1 className="text-xl font-black text-slate-800 flex items-center gap-2">
          <Users className="w-5 h-5 text-indigo-600" /> Ringkasan Aktiviti & Laporan Anak
        </h1>
      ...
      {/* MODAL POPUP LAPORAN ASAL YANG DIKEKALKAN */}
