...
export default function LessonPage() {
  ...
  const runQuizGeneration = async (numQ) => {
    ...
    try {
      ...
      // 🚨 PERBAIKAN UTAMA: Tukar daripada > 0 kepada >= numQ
      // Jika bank soalan database kurang daripada jumlah diminta (e.g., cuma ada 8), kita paksa AI buat set 10 / 20 yang penuh!
      if (rawBankQuestions && rawBankQuestions.length >= numQ) {
        ...
        const lang = getLanguageMode();
        // Paksa LLM menjana kuiz baharu dengan jumlah soalan yang TEPAT mengikut kehendak butang UI (numQ)
        const res = await base44.integrations.Core.InvokeLLM({
          model: "gemini_3_flash",
          prompt: `Based on topic: "${topic?.name || 'General'}", generate EXACTLY ${numQ} high-quality educational multiple-choice questions (MCQs) in ${lang === 'en' ? 'English' : 'Bahasa Melayu'}. It is CRITICAL that you return exactly ${numQ} question items in the JSON array list. Do not cut off.`,
