  ...
  return (
    <div className="bg-stone-50 p-4 rounded-2xl border-2 border-dashed border-emerald-200 flex flex-col items-center space-y-4">
      ...
      <div className="flex w-full gap-3 max-w-[300px]">
        <Button variant="outline" onClick={clearCanvas} disabled={isVerifying} className="flex-1 rounded-xl text-xs font-bold text-red-600 border-red-200 hover:bg-red-50 hover:text-red-700">
          <Eraser className="w-4 h-4 mr-1" /> Padam
        </Button>
        <Button onClick={handleVerify} disabled={isVerifying} className="flex-1 rounded-xl text-xs font-bold bg-emerald-600 hover:bg-emerald-700 text-white">
...
export default function QuizPage() {
  ...
  // 🌟 KEKALKAN MOD FULLSCREEN APABILA MASUK SKRIN KUIZ KHAS
  useEffect(() => {
  ...
  // 🌟 BARU: LOGIK MENGHANTAR KUIZ MENGGUNAKAN EDGE FUNCTION
  const handleSubmit = async () => {
    ...
    try {
      ...
      // Hantar ID Kuiz dan jawapan pelajar ke Edge Function yang selamat
      const response = await base44.functions.invoke("submit-quiz", {
        body: {
          quizId: quizId,
          topicName: quiz?.topic_name || "Topik",
          subjectName: quiz?.subject_name || "Subjek",
          studentAnswers: answers // Cth: { 0: "A", 1: "B" }
        }
      });

      if (response.error) throw new Error(response.error);

      const finalAttemptId = response.data.attemptId;
      ...
      // Halakan ke skrin keputusan
      navigate(`/quiz-result/${finalAttemptId}`);

    } catch (err) { 
      setSubmitted(false); 
      console.error(err);
      alert("Gagal menghantar kuiz. Sila pastikan internet anda stabil dan cuba sekali lagi."); 
    }
  };
    ...
    <div className="max-w-2xl mx-auto px-4 py-6 space-y-6 font-sans bg-[#FAFAF7] min-h-screen">
      ...
      <div className="flex items-center gap-3">
        ...
        {currentQ < questions.length - 1 ? ( 
          ...
          >
            {submitted ? ( <><Loader2 className="w-4 h-4 animate-spin mr-2" /> Memeriksa...</> ) : ("Hantar Kuiz Boss! ⚔️")}
          </Button> 
        )}
      </div>
